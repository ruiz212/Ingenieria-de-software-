import { carrito, agregarAlCarrito, modificarCantidad, eliminarDelCarrito, calcularSubtotal, calcularTotales, calcularDesgloseCambio, vaciarCarrito, setCarrito } from "./pos/cart.js";

// Make cart functions available to window for inline onclick attributes
window.modificarCantidad = (id, delta) => { modificarCantidad(id, delta); renderizarCarrito(); };
window.eliminarDelCarrito = (id) => { eliminarDelCarrito(id); renderizarCarrito(); };
window.agregarAlCarrito = (id, nombre, precio, cat, det) => { agregarAlCarrito(id, nombre, precio, cat, det); renderizarCarrito(); };
window.vaciarCarrito = () => { vaciarCarrito(); renderizarCarrito(); };
window.calcularDesgloseCambio = calcularDesgloseCambio;
...
function initModule1() {
    ...
    if (btnVerPedidosClientes) {
        ...
        // Polling inteligente (Exponential Backoff) para notificaciones
        let pollInterval = 60000;
        const maxPollInterval = 300000; // 5 minutos máximo
        ...
        async function checkCotizaciones() {
            try {
                ...
                if (data.length > 0) {
                    ...
                    pollInterval = Math.min(pollInterval + 30000, maxPollInterval); // Incrementa 30s hasta 5m
                }
            } catch (e) {}
            setTimeout(checkCotizaciones, pollInterval);
        }
        setTimeout(checkCotizaciones, pollInterval);
